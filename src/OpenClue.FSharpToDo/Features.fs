...
let private handleCommand (state: Todo) (cmd: TodoCommand) = Todo.decider.decide state cmd
...
module AssignTodo =
    ...
    type Dto = { AssigneeId: string }

    let private buildAssignTaskCommand (dto: Dto) =
        let cmd =
            result {
                let! assignee = UserId.fromString dto.AssigneeId

                let assignTaskArgs: AssignTodoArgs = { Assignee = assignee }

                return TodoCommand.AssignTodo assignTaskArgs
            }
    ...
    let handle state cmdDto =
        cmdDto
        |> buildAssignTaskCommand
        |> Result.bind (handleCommand state)
        |> Result.bind (handleEvents state)
